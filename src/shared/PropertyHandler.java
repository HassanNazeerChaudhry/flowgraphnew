...
public class PropertyHandler {
    ...
    public static void writeSpacePerformanceLog(String msg) {
        ...
        try {
            ...
            if (i == Integer.parseInt(PropertyHandler.getProperty("numOfWorkers"))){
                ...
                pr1.waitFor();
                System.out.println("finished GC: " + msg + " " + UtilLib.getUsedMemoryMB());
            }
    ...
    private PropertyHandler() throws IOException {
        ...
        try{
            inputStream = new FileInputStream(UtilLib.getJarFolder()+DEFAULT_PROP_LOCATION1);
        ...
        if (inputStream == null) {
            ...
            if (inputStream == null) {
                throw new IOException("config.properties not found on " + UtilLib.getJarFolder()+DEFAULT_PROP_LOCATION1 + " an not found on " + DEFAULT_PROP_LOCATION2);
            }
        }

        prop.load(inputStream);
        inputStream.close();
        UtilLib.disableWarning();
    ...
    private static void finalizeLog(){
        ...
        try {
            File file = new File(PropertyHandler.getProperty("logPath") + PropertyHandler.getProperty("performanceLogName") + UtilLib.getLocalIP().replaceAll("\\.", "_") + ".log");
