...
public class PartitionStreamsHandler {
    ...
    public void solveOperationChain () throws Exception {
        ...
        for (Operations operation:this.operationsList) {
            ...
            if (operation instanceof Operations.Extract) {
                ...
                if (operation instanceof Operations.Map) {
                    ...
                    if (collectedResult.get(0) instanceof Tuple1) {
                        ...
                        if (tuple1.f0.length != 1)
                            throw new RuntimeException("Tuple for evaluation must be single value");
                    ...
                    partitions.getAllElements().values().forEach(partition -> partition.set(partition.filter(opFilter.filterFunction, opFilter.fieldNames)));

                } else if (operation instanceof Operations.FlatMap) {

                    Operations.FlatMap opFlatMap = (Operations.FlatMap) operation;
                    partitions.getAllElements().values().forEach(partition -> partition.set(partition.flatmap(opFlatMap.mapper, opFlatMap.fieldNames)));

                } else if (operation instanceof Operations.GroupBy) {

                    Operations.GroupBy opGroupBy = (Operations.GroupBy) operation;
                    partitions.getAllElements().values().forEach(partition -> partition.set(((ExtractedStream) partition).groupby(opGroupBy.groupingLabels)));
    ...
    private Variable emitVariable(String variableName, long persistence, MultiKeyMap<ExtractedIf> partitions){
        /*
            ...
            return Variable
         */
        ExtractedStream partition;

        try {
            partition = (ExtractedStream)partitions.getAllElements().entrySet().iterator().next().getValue();

        } catch (NullPointerException| NoSuchElementException e) {
            //no partitioning/elements -> skip emission
            return null;
        }
