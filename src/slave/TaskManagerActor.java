...
public class TaskManagerActor extends AbstractActorWithStash implements ComputationCallback, StreamProcessingCallback {
	...
	private ArrayList<SequenceHandler> sequenceHandlers;
	private ArrayList<Event> sentSequenceEvents;
	...
	private TaskManagerActor(String name, String masterAddress) {
		...
		this.sentSequenceEvents = new ArrayList<>();
		this.sequenceHandlers = new ArrayList<>();
	}
...
//				match(InstallSequenceMsg.class, x->stash()).
		    build();
	}

	private final Receive initializedState() {
		return receiveBuilder().
				match(InstallSequenceMsg.class, this::onInstallSequenceMsg). //
...
//		log.info(initMsg.toString());
			...
			for (int i = 0; i < numberOfLines-ignoreLastRecords; i++) {
				if (!line.isBlank()) {
					...
					if ( parsed instanceof AddEdgeMsg && getActor(((AddEdgeMsg)parsed).getSourceName()).compareTo(self()) == 0 ) {
						...
						//ex: parsed = "Insert Vertex: v912345 1"
	...
	//forwards the pattern received from the master to the sequenceHandler
	private final void onInstallSequenceMsg(InstallSequenceMsg msg) {
//		log.info(msg.toString());
//		if(this.sequenceHandler==null)
		this.sequenceHandlers.add(new SequenceHandler(msg.getParameters(), msg.getWindows(), msg.isFull()));
//		else
//			this.sequenceHandler.addPatternElements(msg.getParameters(), msg.getWindows());
		master.tell(new AckMsg(), self());
	}

	private final void  applySequence(Event.EventEnum type, HashMap<String, String> attributes, Long timestamp){
		Event newEvent = new Event(type,attributes,timestamp);
		for(SequenceHandler sequenceHandler:this.sequenceHandlers){
			ArrayList<ArrayList<Event>> result = sequenceHandler.apply(newEvent);
			if(result.size()>0) {
				filterSentResults(result);
				master.tell(new AckMsgResultsSequence(result,sequenceHandler.isFullPattern()), self());
				//todo: if there can be the case of one element belonging to more subpatterns remove break;
				break;
			}
		}
    ...
    private void filterSentResults(ArrayList<ArrayList<Event>> result) {
	    result.forEach(r-> {
	        r.removeIf(event -> this.sentSequenceEvents.contains(event));
           this.sentSequenceEvents.addAll(r);
        });
    }
	...
	private void updateVertexOperations(UpdateVertexMsg msg) {
		...
		if(this.sequenceHandlers.size()>0){
			if(msg.isInsertion()) applySequence(Event.EventEnum.VERTEX_ADDITION,
					getVertexPayload(msg.getAttributes(),msg.getVertexName()),msg.getTimestamp());
	...
	private void updateEdgeOperations(UpdateEdgeMsg msg) {
		...
		if(this.sequenceHandlers.size()>0 )
        	applySequence(Event.EventEnum.EDGE_UPDATE,
					getEdgePayload(msg.getAttributes(),msg.sourceId,msg.destId), msg.getTimestamp());
	}
...
//		log.info(msg.toString());
		...
		if(this.sequenceHandlers.size()>0 )
        	applySequence(Event.EventEnum.EDGE_ADDITION,
					getEdgePayload(msg.getAttributes(),msg.getSourceName(),msg.getDestinationName()), msg.getTimestamp());
		master.tell(new AckMsg(), self());
		debugTaskManagerState();
	}
...
//		log.info(msg.toString());
		...
		if(this.sequenceHandlers.size()>0 ){
			...
			applySequence(Event.EventEnum.EDGE_DELETION,payload, msg.getTimestamp());
		}
...
//		log.info(msg.toString());
		...
		if(this.sequenceHandlers.size()>0 ){
			...
			applySequence(Event.EventEnum.VERTEX_DELETION,payload, msg.getTimestamp());
		}
	}
...
//		log.info(msg.toString());
	...
	public ActorRef getActor(String vertex) {
		//get the actorRef for that partition
	...
	public void registerComputationResult(String vertexName, Tuple2<String, Long> variableName, String[] values, @Nullable Map<String, String> partition) { //Update
		...
		try {
			//todo: add here the result of computation detection
